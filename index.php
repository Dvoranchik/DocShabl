...
class TemplateWord
{
	...
	protected $save;
		...
		$this->save = $params;
		$this->parameters = array_combine($val, array_values($params));
	}
	...
	function creat() {
		...
		copy($this->path, './result.docx');
		if ($docx->open('result.docx') === true) {	
			$xml = $docx->getFromName('word/document.xml');
			$xml = $tem->build($xml, $this->save);
			print_r($xml);
...
$params = array(
    "first"    => 'Малинников',
	"second" => 'Dan',
	"list" => array( 0 => array('man' => "Daniil") ,1 => array('man' => "Borodin"),2 =>  array('man'=>"Vadimovich"))
);

$test = new TemplateWord("path/Shabl.docx", $params);
$test->creat();
